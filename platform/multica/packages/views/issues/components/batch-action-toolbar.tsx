...
export function BatchActionToolbar() {
  ...
  const handleBatchUpdate = async (updates: Partial<UpdateIssueRequest>) => {
    try {
      ...
      toast.success(`Updated ${count} mission${count > 1 ? "s" : ""}`);
    } catch {
  ...
  const handleBatchDelete = async () => {
    try {
      ...
      clear();
      toast.success(`Deleted ${count} mission${count > 1 ? "s" : ""}`);
    } catch {
      toast.error("Failed to delete missions");
    } finally {
      setDeleteOpen(false);
    }
  };
    ...
    <>
      ...
      <AlertDialog open={deleteOpen} onOpenChange={setDeleteOpen}>
        <AlertDialogContent>
          <AlertDialogHeader>
            <AlertDialogTitle>
              Delete {count} mission{count > 1 ? "s" : ""}?
            </AlertDialogTitle>
            <AlertDialogDescription>
              This action cannot be undone. This will permanently delete the
              selected mission{count > 1 ? "s" : ""} and all associated data.
            </AlertDialogDescription>
          </AlertDialogHeader>
          <AlertDialogFooter>
