    ...
    <div className="space-y-4">
      ...
      {agent.skills.length === 0 ? (
        <div className="flex flex-col items-center justify-center rounded-lg border border-dashed py-12">
          ...
          <p className="mt-3 text-sm text-muted-foreground">No doctrines assigned</p>
          <p className="mt-1 text-xs text-muted-foreground">
            Add doctrines to share team knowledge with this agent. Local skills are already used automatically.
          </p>
      ...
      {showPicker && (
        <Dialog open onOpenChange={(v) => { if (!v) setShowPicker(false); }}>
          <DialogContent className="max-w-md">
            <DialogHeader>
              <DialogTitle className="text-sm">Add Doctrine</DialogTitle>
              <DialogDescription className="text-xs">
                Select a doctrine to assign to this agent.
              </DialogDescription>
            </DialogHeader>
