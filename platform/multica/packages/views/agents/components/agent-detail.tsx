...
import { getRoleBadgeClass } from "../bope-config";
import { resolveBopeAgent } from "../bope-roster";
...
export function AgentDetail({
  ...
  const bopeMeta = resolveBopeAgent(agent);
    ...
    <div className="flex h-full flex-col">
      ...
      <div className="flex min-h-12 shrink-0 items-center gap-3 border-b px-4 py-2">
        ...
        <div className="min-w-0 flex-1">
          ...
          {agent.description && !isArchived && (
            <p className={`mt-0.5 w-fit rounded px-1 py-0.5 text-xs font-medium ${getRoleBadgeClass(agent)}`}>
              {agent.description}
            </p>
          )}
          {bopeMeta && !isArchived && (
            <>
              <p className="mt-1 text-xs text-muted-foreground">
                {bopeMeta.doctrinalFunction}
              </p>
              {(bopeMeta.rank || bopeMeta.medal) && (
          ...
          <DialogContent className="max-w-sm" showCloseButton={false}>
            <div className="flex items-center gap-3">
              ...
              <DialogHeader className="flex-1 gap-1">
                <DialogTitle className="text-sm font-semibold">Archive member?</DialogTitle>
